...
def Anvisa(tipo_req, produto):
    ...
    # Executando uma consulta
        cur.execute("SELECT public.precos_maximos.ean, public.precos_maximos.nome, public.precos_maximos.principio_ativo, public.precos_maximos.categoria, public.precos_maximos.lista, public.cest.cest FROM public.precos_maximos LEFT JOIN public.cest ON public.precos_maximos.id_cest = public.cest.id_cest WHERE public.precos_maximos.nome ILIKE '%s' LIMIT 1;" % (produto))
    else:
        return 'Tipo de dado inválido, use nome ou barras'
    # Obtendo os resultados
    rows = cur.fetchall()
    if len(rows) == 0:
        cur.execute("SELECT * FROM public.smerp_meds WHERE nome_smerp ILIKE '%s' LIMIT 1;" % (produto))
    ...
    if isinstance(produto, int):
        ...
        try:
            ...
            if link:
                link.click()
            try:
                registro = driver.find_element(By.XPATH, '//*[@id="anvisa"]/div[1]/div[2]/div[2]').text
                ...
                if len(qrcest) == 0:
                    ...
                    cest = qrcest[0][0]
                try:
                    cur.execute("INSERT INTO smerp_meds VALUES (%s, %s, %s, %s, %s, %s, %s);", (registro, nome, p_ativo, categoria, lista, ncm, cest))
                    conn.commit()
